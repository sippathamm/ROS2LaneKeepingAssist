...
class StreamPublisherCamera : public rclcpp::Node 
{
public:
  StreamPublisherCamera() : Node("stream_publisher_camera")
  {
    this->declare_parameter("stream_source_index", 0);
    this->declare_parameter("silence", false);

    const int StreamSourceIndex = this->get_parameter("stream_source_index").as_int();
    this->Silence_ = this->get_parameter("silence").as_bool();
    
    this->VideoCapture_ = cv::VideoCapture(StreamSourceIndex);
    ...
    this->Timer_ = this->create_wall_timer(33.33ms, std::bind(&StreamPublisherCamera::TimerCallback, this));
  }
...
int main (int argc, char *argv[]) 
{
  ...
  auto node = std::make_shared<StreamPublisherCamera>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
