...
class SteeringPredictor(Node):
    def __init__(self):
        ...
        try:
            ...
            raise FileNotFoundError(
                f'{colors.ERROR}[ERROR] The ONNX model file could not be found. '
                f'Received: model_path={self.MODEL_PATH}{colors.ENDC}'
            )

        self.get_logger().info(f'{colors.OKGREEN}> Initialized without any errors. Waiting for image...{colors.ENDC}')
    ...
    def image_callback(self, msg):
        ...
        normalized_steering_angle = min(max(normalized_steering_angle, -1.0), 1.0)

        cmd_steering_angle = Int16()
        cmd_steering_angle.data = int((normalized_steering_angle - 1.0) *
        ...
        cmd_speed.data = int(k * (self.MAX_SPEED - self.MIN_SPEED) * normalized_steering_angle + self.MAX_SPEED)
        ...
        cv2.putText(background, 'Predicted steering angle: ' + str(normalized_steering_angle), (30, 150),
